...
- ✅ Demo command (v0.2.0, unscoped package): **`npx shipsafe-voyageblack demo`** — Hormuz path, window 14:57→15:02Z. Dashboard also has a **"Load Auth Outage Demo"** button (non-maritime, 11-event auth→payment→notification cascade) — great universality shot.
...
| 0:12–0:35 | Timeline highlights **VoyageBlack · 14:57 · "Incident window opened — logs ingesting."** Cut to a wall of raw log lines scrolling — ERROR, CRITICAL, timeouts — unreadable. | "Here's the problem VoyageBlack solves, in plain terms: after a serious outage, someone has to write the postmortem — reconstruct what happened across thousands of log lines and a dozen services. It takes weeks, so it usually gets skipped — and the same failure happens again." |
| 0:35–0:55 | Click **VoyageBlack → Open**; the dashboard with "Generate Postmortem" loads. | "VoyageBlack writes it in about ninety seconds. It reads the logs from Elastic, uses Gemini to rebuild the timeline and find the root cause, and even recalls similar past incidents — then a human approves before anything is saved." |
| 0:55–1:15 | Dashboard: "Load Hormuz Demo" + "Generate Postmortem". | "Why Gemini? It does what search alone can't: synthesize scattered events into a single causal story and write recommendations grounded in the actual evidence. The engine underneath is Elastic — ELSER semantic search plus ES|QL." |
