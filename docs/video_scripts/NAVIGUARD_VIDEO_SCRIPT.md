...
| 0:12–0:35 | Timeline highlights **NaviGuard · 15:00 · "Routing model crisis avoidance −41% BLOCK."** Cut to a green "Overall accuracy 80% ✓" tile beside a collapsed `crisis_avoidance` bar. | "Here's the problem NaviGuard solves, in plain terms: an AI model in production can quietly get *worse* at one specific job while its overall score still looks healthy — and no alarm fires. Here, a ship-routing model's confidence in avoiding danger zones silently collapsed, but the dashboard stayed green. You'd only find out when a ship sailed into a restricted strait." |
