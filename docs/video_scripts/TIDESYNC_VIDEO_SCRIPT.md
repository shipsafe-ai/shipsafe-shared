...
| 0:12–0:35 | Timeline highlights **TideSync · 14:59 · "Jebel Ali sync failing — 3 reports stale."** Cut to a Fivetran tile, big green "Sync succeeded · 2m ago ✓", beside a BigQuery result `MAX(_fivetran_synced) = 04:33` while now is `08:59`. | "Here's the problem TideSync solves, in plain terms: your data pipeline can report 'success, all green' while the actual data is hours stale — because it synced *nothing*, successfully. The dashboards look fine; the reports are quietly wrong; and nobody knows, because 'did it run' and 'is the data fresh' are two different questions." |
| 0:35–0:55 | Click **TideSync → Open**; the split Contradiction panel loads. | "TideSync is the agent that catches it. It checks the pipeline's status *and* queries the warehouse directly for how fresh the data really is — and when the two disagree, Gemini explains the gap and the business impact. A human approves any fix." |
| 0:55–1:08 | TideSync Contradiction panel; hover the **Run** button. | "Two independent sources of truth: the Fivetran control plane for status, and BigQuery directly for freshness. Gemini reasons about the gap between them — and what stale data costs the business." |
