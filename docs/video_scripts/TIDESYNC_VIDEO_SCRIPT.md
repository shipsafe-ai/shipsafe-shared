...
| **2:05–2:30** | **ImpactMapper (Gemini)** verdict card: `is_stale`, `lag_display`, `breach_confidence`, business `recommendation`. A deterministic Critic check sits upstream. | "This is where Gemini reasons. ImpactMapper takes both reports and judges: is this data still trustworthy, how confident are we it's a real breach, and what's the business impact — which downstream reports are now built on stale numbers. Before it runs, a deterministic scanner checks the inputs for injection. The verdict is structured, not prose." |
| **2:30–2:50** | `status: awaiting_approval` + **"Approve Resync"** button with "Human approval required before sync." Click it → `POST /approve/{token}` → RecoveryAgent runs Fivetran setup tests, then triggers a sync. | "And it does not fix it behind your back. TideSync found the stale feed and proposed the recovery — a re-sync — but every write to Fivetran is gated. It returns 'awaiting approval' with a token. The operator approves; only then does the RecoveryAgent run the setup tests and trigger the sync. The machine recommends. You decide." |
| 2:50–3:08 | One slide: `port feed → finance close → ML feature store → patient records`. | "TideSync is demonstrated on a stale shipping feed. The pattern is universal: a finance close built on yesterday's numbers, an ML feature store quietly frozen, a patient record that didn't update. Anywhere a pipeline can succeed while the data goes stale, TideSync is the cross-check." |
